...
describe("legacy migration", () => {
  ...
  it("copies legacy data into the new Mnemonic directories", async () => {
    ...
    const paths = {
      ...
      lanceDir: join(root, "new-data-root", "data", "lancedb"),
      settingsPath: join(root, "new-config-root", "settings.json"),
      logsDir: join(root, "new-data-root", "logs"),
      ipcDir: join(root, "new-data-root", "ipc"),
      ipcStatusPath: join(root, "new-data-root", "ipc", "status.json"),
      ipcEventsPath: join(root, "new-data-root", "ipc", "events.ndjson"),
  ...
  it("does not overwrite an existing new installation", async () => {
    ...
    const paths = {
      ...
      lanceDir: join(dataRoot, "data", "lancedb"),
      settingsPath: join(configRoot, "settings.json"),
      logsDir: join(dataRoot, "logs"),
      ipcDir: join(dataRoot, "ipc"),
      ipcStatusPath: join(dataRoot, "ipc", "status.json"),
      ipcEventsPath: join(dataRoot, "ipc", "events.ndjson"),
